#define MODO_SIMULACION true  // ← cambia a false cuando tengas el hardware real
...
void taudio(void *parameter) {
  ...
  while (true) {
    ...
    for (int i = 0; i < BUFFER_SIZE; i++) {
      ...
      switch (waveform) {
        case 0: val = sinf(fase); break;
        case 1: val = fase < PI ? 1.0f : -1.0f; break;
        case 2: val = 2.0f * fabs(2.0f * (fase / (2.0f * PI)) - 1.0f) - 1.0f; break;
      }
...
void tsensor(void *parameter) {
  ...
  while (true) {
    ...
    vTaskDelay(pdMS_TO_TICKS(500)); //APP
  }
}
